...
OUT_PATH = os.path.join("models", "data_audit_report.txt")
...
total_rows = len(df)
print(f"  Loaded {total_rows:,} rows x {len(df.columns)} columns", flush=True)
...
if FAILURE_COL not in df.columns:
    ...
    for src in src_counts.index:
        ...
        if fail_rate < FAILURE_MIN_RATE:
            out(f"    WARNING: FAILURE RATE BELOW {FAILURE_MIN_RATE}% -- not contributing failure signal")
        ...
        for mode, cnt in mode_counts.items():
            ...
            dom_flag = " WARNING: DOMINANT (>50%)" if mode_pct > FAILURE_DOM_PCT else ""
...
df_sorted = (
    df[[GROUP_COL, SOURCE_COL, "timestamp"]]
    .sort_values([GROUP_COL, "timestamp"])
...
if broken_pct > 10:
    out(f"  WARNING: >10% broken sequences -- cross-source boundary contamination is HIGH")
    boundary_flag = True
elif broken_pct > 1:
    out(f"  WARNING: {broken_pct:.1f}% broken sequences -- moderate boundary contamination")
    boundary_flag = True
else:
    out(f"  OK: Boundary contamination is low (<1%)")
    boundary_flag = False
